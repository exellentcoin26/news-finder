...
pub async fn scrape_rss_feed(feed: &PrismaFeed, client: &PrismaClient) -> Result<()> {
    let source_id = feed.source_id;
    let rss_category = feed.category.clone();
    let rss_feed = feed.feed.clone();
    ...
    let feed = match get_rss_and_validate(&rss_feed).await {
        ...
        Err(_) => {
            ...
            log::error!("Rss feed invalid or (temporarily) not reachable (rss feed: `{rss_feed}`)");
            return Ok(());
        }
    };
    ...
    for (label, article_url) in labels_with_articles {
        ...
        label_batch.push(client.news_article_labels().upsert(
            prisma::news_article_labels::UniqueWhereParam::IdLabelEquals(article.id, label.clone()),
